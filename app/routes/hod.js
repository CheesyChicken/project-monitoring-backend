...
router.get('/hodg/:clg/:dep', (req, res, next) => {
    var cllg = req.params.clg;
    var depa = req.params.dep;
    var sql = "select a.* ,b1.FullName, b2.FullName as guide2,c1.College_Name as clg,s1.Status_Name as Status ,dd1.Department_Name,d1.Domain_Name as dom1,d2.Domain_Name as dom2,d3.Domain_Name as dom3,d4.Domain_Name as dom4,pt.Project_Type_Name as TpOfProject,Co.Company_name from project_group a Join department dd1 on dd1.Department_id=a.Department_id  JOIN persons b1 on b1.Person_id=(select person_id from instructor where Instructor_id=a.Instructor_id1) Join persons b2 on b2.Person_id =(select person_id from instructor where Instructor_id=a.Instructor_id2) Join college c1 on c1.College_id=a.College_id Join project_domain d1 on d1.Domain_id=a.Domain_Pref_1 Join project_domain d2 on d2.Domain_id=a.Domain_Pref_2 Join project_domain d3 on d3.Domain_id=a.Domain_Pref_3 Join project_domain d4 on d4.Domain_id=a.final_domain Join status s1 on s1.Status_id=a.Status_id join project_type pt on pt.Project_Type_id=a.Project_Type_id join Company Co on Co.Company_id=(select Company_id from industryproject where industry_project_id=a.industry_project_id)  where a.College_id=? and a.Department_id=?"
    connection.query(sql, [cllg, depa], (err, task, fields) => {
        if (err) {
            res.status(400).json({
                status: 'error',
                error: err,
